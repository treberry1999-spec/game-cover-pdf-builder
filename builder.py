...
def load_moby_map():
    ...
    if not pages.exists():
        return MOBY_MAP
    title_marker='<meta property="og:title" content="'
    platform_marker=":platforms='"
    for fp in pages.glob("*.html"):
        try:
            txt=fp.read_text(encoding="utf-8",errors="ignore")
            a=txt.find(title_marker)
            if a < 0:
                continue
            a += len(title_marker)
            b=txt.find('"',a)
            if b < 0:
                continue
            title=html.unescape(txt[a:b]).replace(" - MobyGames","").strip()
            p=txt.find(platform_marker)
            if p < 0:
                continue
            p += len(platform_marker)
            q=txt.find("'\n      :game-id=",p)
            if q < 0:
                q=txt.find("'\r\n      :game-id=",p)
            if q < 0:
                continue
            arr=json.loads(html.unescape(txt[p:q]))
            key=normkey(title)
            for item in arr:
...
def moby_candidates(row):
    ...
    platform=row.get("platform","")
    if platform not in {"PlayStation 4","PlayStation 5","Nintendo Switch 2"} or not mapping:
        return
    keys=[]
    for t in variants(row):
        k=normkey(t)
        ...
        for item in mapping.get(k,[]):
            if item.get("platform")==platform:
                ...
                if u and u not in seen:
                    seen.add(u)
                    yield u,f"MobyGames mirror {platform}: {item.get('title','')}"
